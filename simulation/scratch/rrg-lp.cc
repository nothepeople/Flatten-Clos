...
void SetMultiPathForAllHosts() {
  //
	ConstructSwitchConnMatrix(sw2sw_mat_file);
	CalSwToSwPath();
	// int total_hosts = switch_num * host_per_switch;
	// InstallPath(49, 22, n);
...
void InstallWeight(int hostA, int hostB) {
	...
	// std::cout << "src_tor: " << src_tor << " dst_tor: " << dst_tor <<  "ksp: " << ksps.size() << " weights: " << weights.size() << "\n";
	NS_ASSERT_MSG(ksps.size() == weights.size(), "the size of weights must equal to that of ksps");
	...
	for (int i = 0; i < ksps.size(); i++) {
		...
		for (int j = 0; j < path.size(); j++) {
			int sw_id = hosts + path[j];
			Ptr<Node> sw =  n.Get(sw_id);
			NS_ASSERT_MSG(sw->GetNodeType()== 1, "sw node type must be 1");
			if (j == 0) {
				DynamicCast<SwitchNode>(sw)->AddRtWeight(srcAddr, dstAddr, rtWeights[src_tor][dst_tor][i]);
			} else {
				DynamicCast<SwitchNode>(sw)->AddRtWeight(srcAddr, dstAddr, 1);
			}
		}
	}
}
