...
def topo_gen(switches, ports, to_hosts, ports_of_vir_layer,
  ...
  print("Write a mat file in topo-gen func...")
  mat_file.write("%d %d %d %d\n" % (to_hosts, switches, (ports - to_hosts) * switches, switches * to_hosts) )
  for i in range(switches):
    for j in range(switches):
      if i==j or topo_matrix[i,j]==0: continue
...
if __name__ == "__main__":
  ...
  topo_matrix, ports_conn_matrix, switch_objs = topo_gen(
    ...
    up_down_topo_dir="topo/up-down/topo.txt",
    up_down_trace_dir="trace/up-down/trace.txt",
  ...
  path_dir = "topo/up-down/path.txt"
  path_file = open(path_dir, mode='w')
  ...
  ## WRITE EDST PATH
  print("Write EDST PATH...")
  bg = time.time()
  os.system("g++ edst.cc -w -std=c++11 -o edst")
  os.system("./edst")
  ed = time.time()
  print("EDST runs %.2fs" % (ed - bg) )
